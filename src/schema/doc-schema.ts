...
export const docSchemaRelativePath = "docs/.schema/feature-page.md";

export const defaultDocSchema = `# Commit Doc Agent Feature Page Schema
...
export async function ensureProjectDocSchema(repoPath: string): Promise<string> {
  const schemaPath = resolve(repoPath, docSchemaRelativePath);
  ...
  await mkdir(resolve(repoPath, "docs", ".schema"), { recursive: true });
  await writeFile(schemaPath, defaultDocSchema, "utf8");

  return schemaPath;
}

export async function loadProjectDocSchema(repoPath?: string): Promise<string> {
  if (!repoPath) {
    return defaultDocSchema;
  }

  const schemaPath = await ensureProjectDocSchema(repoPath);
